...
% cd(originalpath)
% input_path = path + string(StartingFileName(i).name)
% dataArray = {};
% name_file = {};
for i =1:length(StartingFileName)
    tic
    copyfile([path, char(StartingFileName(i).name)])
...
%             end
    ...
    file_name = strsplit(string(StartingFileName(i).name),'.plx');
    save_name_data =([string('../Output files/'),string(file_name{1}),'_lfp_data.mat']);
    save_name_head =([string('../Output files/'),string(file_name{1}),'_lfp_head.mat']);
    

    save(save_name_data,'lfp_data');
    save(save_name_head,'adgains','adnames','adfreq','-v6'); 
    %save(save_name_head,'-regexp', '^(?!(allad|lfp_data)$).','-v6'); 
    
    f = toc;
    toc;
    %disp(['Tempo gasto em ',string(StartingFileName(i).name),': ',char(f)]) 
    
    fileID = fopen([string('../Output files/'),string(file_name{1}),'_lfp_Tempo.txt'],'w');
    fprintf(fileID,'Tempo gasto: %s',f);
    fclose(fileID);
