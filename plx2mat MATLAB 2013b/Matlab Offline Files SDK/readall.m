...
%             end
    ...
    save_name_data =([string('../Output files/'),string(file_name{1}),'_data.mat']);
    save_name_head =([string('../Output files/'),string(file_name{1}),'_head.mat']);
    
    save(save_name_data,'');
    save(save_name_head,'-regexp', '^(?!(allad)$).','-v6'); 
    
    f = toc;
    toc;
    % disp(['Tempo gasto em ',string(StartingFileName(i).name),': ',toc]) 
